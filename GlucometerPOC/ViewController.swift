...
class ViewController: UIViewController, BLEProtocol, UIPickerViewDelegate, UIPickerViewDataSource {
    
    // Globals
    
    let TEST_MODE = false

    var glucoseDataset: GlucoseData = GlucoseData() // Handle to the Model
    ...
    @IBOutlet weak var pickerView: UIPickerView!// one line text display for status
    ...
    func BLEfoundPeripheral(device: CBPeripheral, rssi: Int) {
        ...
        if (device.identifier != UUID(uuidString: glucometerDeviceID)) {
            pickerView.reloadAllComponents()
            showPickerView()
            return
            
        } // Only default device for now. Later add a table view and picker so the user can select any device
    ...
    @IBAction func doneButtonPressed() {
        ...
        let selectedPeripheral = peripherals[selectedIndex]
        print("Slected device is \(selectedPeripheral.name)")
            hidePickerView()
        glucometer = selectedPeripheral
        textDisplay.text.append("Connecting to device\n")
        textStatus.text = "Connecting to glucometer"
        bluetoothHandle.connect(peripheral: glucometer)
    }
    ...
    func showPickerView() {
        UIView.animate(withDuration: 0.2, animations: { () -> Void in
            self.pickerHeight.constant = 192
